...
const MyProfile = () => {
  ...
  const { data:currentUser ={}, isLoading } = useQuery({
    ...
    const approvedLoans = myLoans.filter(loan=>loan.status === 'approved').length
    const rejectedLoans = myLoans.filter(loan=>loan.status === 'rejected').length
    const pendingLoans = myLoans.filter(loan=>loan.status === 'pending').length
  ...
  return (
    <div className='min-h-screen bg-gray-100 flex h-auto'>
            <div className='w-64 bg-green-900 flex flex-col items-center py-10'>
                ...
                <div className='mt-10 space-y-4 w-full px-2 md:px-6 flex flex-col'>
                    <Link to={'/'} className='hover:bg-green-700 p-2 rounded cursor-pointer text-white flex
                    items-center gap-2'><FaHome />Home</Link>
                    <Link to={'/dashboard/manage-loans'} className='hover:bg-green-700 p-2 rounded cursor-pointer text-white flex
                    items-center gap-2'><TbMoneybag />Loans
                    </Link>
                    <p className='hover:bg-green-700 p-2 rounded cursor-pointer text-white flex
            ...
            <div className='flex-1 p-4 md:p-8'>
                <h1 className=' text-lg md:text-2xl font-bold mb-6 text-secondary custom-font'>Users Dashboard</h1>
                    ...
                    <div className='bg-primary text-white p-2 md:p-6 rounded-lx shadow text-center rounded-xl mx-auto md:mx-0 w-50 ml-5 '>
                            ...
                            <h2 className='tex-lg md:text-2xl font bold custom-font'>{totalLoans}</h2>
                    </div>
                    ...
                    <h2 className='text-lg font-semibold mb-4 text-secondary mt-20'>Loan Activity</h2>
                        <div className='bg-white  rounded-xl shadow grid grid-cols-1  gap-6'>
                ...
                <div className='mt-8 p-6 rounded-xl shadow'>
                    <h2 className='text-secondary text-2xl font-semibold'>Profile Info</h2>
